...
def uploadFile(filename, folderID=None):
	service = authenticateGoogleDrive()

	fileMetadata = {'name': filename}

	# Unique Folder to be uploaded to, comment if not available
	if folderID:
		fileMetadata['parents'] = [folderID]
	
	media = MediaFileUpload(filename, mimetype='application/x-7z-compressed')
